...
class Enrolment extends Eden {
	...
	public function setPaths() {
		$this->registry()
			...
			->set('path', 'model', 	$this->_root.'/model')
			->set('path', 'config', 	$this->_root.'/config')
			->set('path', 'theme', 		$this->_root.'/theme')
			->set('path', 'page',		$this->_root.'/enrolment/page')
			->set('path', 'template',	$this->_root.'/enrolment/template')
	...
	public function setRequest() {
		...
		if(!$page) { 
			...
			while(count($buffer) > 1) {
				...
				$class = 'Enrolment_Page'.str_replace(' ', '_', $parts);
	...
	public function template($file, array $data = array()) {
		Enrolment_Error::i()->argument(1, 'string');
	...
	public function error($error, $event, $type, $level, 
		...
		echo Eden_Template::i()
			...
			->set('line', $line)
			->set('message', $message)
			->parsePhp(dirname(__FILE__).'/enrolment/error.phtml');
	}
